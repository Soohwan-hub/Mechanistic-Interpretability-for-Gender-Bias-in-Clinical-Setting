...
| `fig9_cot_residual_by_condition_promptA.png` | CoT rewrite score, layer × token, four conditions (depression, multiple sclerosis, rheumatoid arthritis, sarcoidosis), Qwen Prompt A. Top-15 tokens per condition by max|rewrite score|, all 28 layers, shared symmetric [-1, 1] colorbar. | `raw_uploads/cot_patching_qwen/mlp_cot_patching_results/prompt_A/<condition>/heatmap_data_<condition>.pkl` → `matrix` + `row_labels` |

...
Sources:
- `raw_uploads/log_delta/` — Qwen logprob_delta MLP results (token × layer CSVs; layer 0 already excluded; 31 layers L1–L31)
- `raw_uploads/cot_patching_olmo/mlp_cot_patching_results/` — CoT MLP results (model identified as **OLMo-7B-0424-Instruct** via tokenizer fingerprint; 32 layers L0–L31; rewrite_score per `safe_var_matrix_*.pkl` of shape `(n_patch_targets × 32_layers)`)
...
## §2.6 — Qwen logprob_delta (raw_uploads/log_delta/)

Source: `raw_uploads/log_delta/fig_token_layer_heatmap_all155_logdelta.csv` (overall, 155 units aggregated by the producer). 31 layers (L1–L31), 57 token positions.
...
|---|---:|---:|---:|---:|
| Qwen (raw_uploads/log_delta/, 31-layer source, L0 excluded) | L30 | 0.7914 | L30 | 2.9958 |

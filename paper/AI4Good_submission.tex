...
\usepackage{amsmath}
\usepackage{xcolor}
\definecolor{comment1blue}{RGB}{0,0,200}
\definecolor{comment1bg}{RGB}{220,235,255}

\newif\ifreview
\reviewtrue    % set \reviewfalse before final submit
\usepackage{tabularx}
...
\begin{abstract}
Large language models (LLMs) are increasingly used in clinical decision-making, yet they encode persistent gender biases that may worsen healthcare disparities. Chain-of-Thought (CoT) prompting is widely used to improve diagnostic reasoning, but whether it reduces internal bias or only changes how bias is expressed remains unclear. We apply activation patching and sparse-autoencoder (SAE) ablation to \texttt{Qwen2.5-7B-Instruct} under simple and CoT prompting across five clinical conditions; \texttt{OLMo-7B-0724-Instruct-hf} replicates the simple-prompt patching analysis only. Under simple prompting, gender information is broadly present in the residual stream but causally concentrated at MLP layer~18. Under CoT, the layer-18 effect at the \emph{first} mention of the condition name stays comparable to simple prompting and much stronger than at later mentions---CoT redistributes where bias is accessible, rather than removing it. SAE ablation finds bias-relevant latents under both strategies, with CoT mainly lowering activation strength rather than eliminating these features. CoT is therefore insufficient as a standalone clinical debiasing technique.
\end{abstract}
% ========== ABSTRACT-REVISE-END ==========
...
% ========== COMMENT1-START: NEW §2 Methods (blue in review PDF) ==========
\ifreview\begingroup\fi

\section{Methods}
\label{sec:methods}

\subsection{Models and tasks}
We study \texttt{Qwen2.5-7B-Instruct} (primary) and \texttt{OLMo-7B-0724-Instruct-hf} (simple-prompt replication). Sparse autoencoder (SAE) ablations and all CoT patching analyses use Qwen only. We evaluate five conditions where Qwen over-generates female patients in vignettes: asthma, depression, multiple sclerosis, rheumatoid arthritis, and sarcoidosis. Each condition uses 31 simple prompt templates and 20 chain-of-thought (CoT) templates (10 Prompt~A and 10 Prompt~C with \texttt{<thinking>} tags).

\subsection{Vignette generation and baselines}
For behavioral baselines (Figure~\ref{fig:prop}; Figure~\ref{fig:cot_prop}), we generate patient vignettes with 35 samples per condition$\times$prompt, sampling temperature $0.7$ and \texttt{max\_new\_tokens}$=320$. Gender is classified from generated text using the same keyword-based matcher as in prior vignette work (Ahsan et al., 2025; Zack et al., 2024).

\subsection{Rewrite score and activation patching}
Following Ahsan et al. (2025), we patch activations from a clean Male source prompt into a corrupted clinical vignette prompt at a chosen layer and token position. Let $p(\texttt{Male})$ denote the probability of generating the token \texttt{Male} at the gender-scoring position \emph{before} intervention, and $p_*(\texttt{Male})$ the probability \emph{after} patching. The \textbf{rewrite score} is
\begin{equation}
\label{eq:rewrite}
s = \frac{p_*(\texttt{Male}) - p(\texttt{Male})}{1 - p(\texttt{Male})},
\end{equation}
which measures how effectively the intervention rewrites the model's gender prediction toward male (Hase et al., 2024, as in Ahsan et al.).

For simple prompts, we append a forced \texttt{Gender:} suffix so the next token is \texttt{Male} or \texttt{Female}, matching the prior vignette setup. For each prompt, we run a full layer$\times$token MLP \texttt{down\_proj} sweep (all layers, all token positions). Headline statistics and bar plots report layer~18 scores at the condition-token site (last subtoken for simple prompts); heatmaps show the full sweep. For CoT MLP sweeps, we use the full CoT prompt template (with \texttt{<thinking>} instructions in the user message, not a model-generated reasoning trace), append the same forced \texttt{Gender:} suffix for scoring, and likewise sweep all layers$\times$tokens; because the template names the condition multiple times, headline layer~18 scores are reported at the first and last condition-token mentions separately. Residual-stream patching is reported in Appendix~\ref{app:residual}. When rewrite scores saturate, we also report the log-probability delta $\Delta\log P(\texttt{Male}) = \log p_*(\texttt{Male}) - \log p(\texttt{Male})$ at the same scoring position. Macro-averaging and token-position conventions are in Appendix~\ref{app:reporting}.



\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig1_mech_pipeline.png}
    \caption{Overview of the mechanistic interpretability pipeline: clinical prompting and vignette generation, activation patching with rewrite-score evaluation under simple and CoT prompting, and SAE latent validation.}
    \label{fig:methods_pipeline}
\end{figure}

\ifreview\endgroup\fi
% ========== COMMENT1-END ==========

\section{Localizing patient gender under simple and CoT prompting}

\subsection{Vignette generation}
Prior work showed that LLMs can exaggerate demographic associations in clinical vignette generation (Zack et al., 2024), and later mechanistic analysis demonstrated that under simple prompting these effects can be causally localized using activation patching (Ahsan et al., 2025). We extend that setup by asking whether the same localization behavior holds under Chain-of-Thought (CoT) prompting. We aim to gain insight on whether patient-gender information remains localized to a small set of internal activations under CoT, or becomes more distributed across layers and tokens. These baseline distributions establish where female over-representation is present before intervention and define the target contexts for mechanistic localization (Figure~\ref{fig:prop}; Figure~\ref{fig:cot_prop}).

\begin{figure}[h]
    \centering
    \begin{minipage}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{fig1_gender_probs.png}
...
Layer-wise aggregates are in Appendix~\ref{app:toplayers} (Figure~\ref{fig:simpleaggregate}).

Analyzing results with a more granular (layer, token) pair, the rewrite score metric is much higher than the overall layer aggregate (Figure~\ref{fig:rewrite}), showing the localization of gender bias is highly localized to condition tokens within each layer. Figure~\ref{fig:rewrite} plots Qwen \emph{per-condition} means at layer~18 (e.g., asthma ${\sim}$0.83); the Qwen simple macro-mean over all 155 cells is ${\sim}$0.40 (Appendix~\ref{app:reporting}). OLMo shows the same layer-18 pattern at lower magnitude (${\sim}$0.13 macro over 155 cells). This granular view also reveals the variation of rewrite scores based on condition. Asthma and depression are highly malleable to patching, multiple sclerosis and sarcoidosis show weaker effects, and rheumatoid arthritis sits near the floor of the rewrite score.

\begin{figure}[h]
    \centering
    \includegraphics[width=.75\linewidth]{Model_Comparison_By_Condition.png}
    \caption{Per-condition means of layer-18 condition-token rewrite scores (Qwen, simple prompting). Macro-mean over all 155 cells is ${\sim}$0.40.}
   
    \label{fig:rewrite}
\end{figure}

This disparity in rewrite score across conditions can be further seen by analyzing specific prompt-condition heatmaps (detailed in Appendix~\ref{app:heatmaps}). While asthma shows strong localized activation at layer 18 associated with the condition token, rheumatoid arthritis shows minimal rewrite score changes in the middle layers, which initially suggests that the gender circuit might be absent for certain female-biased diseases.

To validate these findings and compare how the two models localize patient gender under simple prompting, we utilize two metrics. The rewrite score is the probability-space metric in Eq.~\eqref{eq:rewrite} (Section~\ref{sec:methods}). The logprob delta is an unbounded log-probability metric used alongside it. We utilized the logprob delta metric because of less saturation in extremes and added stability for tiny probabilities, which was common in our findings.

The logprob delta confirms layer-18 localization even when rewrite score saturates: rheumatoid arthritis has rewrite ${\sim}$0.012 per-condition mean but a sharp layer-18 logprob peak, so near-zero rewrite scores reflect saturation rather than absent circuitry.

\subsection{Residual stream patching}
To complement the MLP localization, we evaluated the residual stream under both simple and CoT prompting. Unlike the sharp MLP layer-18 peak, residual patching produces a broadly distributed mid-layer plateau (spanning roughly layers 5 through 21) before collapsing sharply to near zero at layer 22. Detailed analysis and corresponding layer-by-layer heatmaps for the residual stream are provided in Appendix~\ref{app:residual}.
...
We applied the activation-patching protocol in Section~\ref{sec:methods} to CoT-prompted generations, run on Qwen2.5-7B-Instruct only. CoT templates name the condition at several token positions, so we report first- vs last-mention layer-18 scores separately (Appendix~\ref{app:reporting}). Example traces are shown in Appendix~\ref{app:vignette_examples}.

\subsection{Mechanistic analysis: CoT results}
\paragraph{MLP intervention.} Under CoT on Qwen, MLP layer-18 rewrite scores at the \emph{first} condition token macro-average ${\sim}$0.44 vs simple ${\sim}$0.40 (100 vs 155 cells; Appendix~\ref{app:reporting}). Last-mention scores macro-average ${\sim}$0.05; full-token averaging yields ${\sim}$0.01 and can falsely suggest circuit removal. Per-condition bars in Figure~\ref{fig:cotmlp_comparison} (e.g., asthma ${\sim}$0.94 vs ${\sim}$0.83) are within-condition means and should not be compared directly to macro-means (Appendix~\ref{app:reporting}). Thus CoT does not erase the early condition-token layer-18 effect. The effect is concentrated at early condition mentions and weaker at later ones. We therefore report first- vs last-condition-token scores as the primary CoT MLP statistic, rather than treating sparse heatmap cells as evidence that the circuit disappeared (Figure~\ref{fig:cotmlp_comparison}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=.85\linewidth]{fig5_simple_vs_cot.png}
    \caption{Qwen layer-18 MLP rewrite scores by condition and macro-means (``ALL''). Simple = condition-token site; CoT = first/last condition-token and full-token mean.}
...
\paragraph{Overview}
Collectively, CoT does not remove the early condition-token layer-18 MLP gender effect; that effect remains comparable to simple prompting, while later condition mentions are weaker. Residual-stream patching under CoT still recovers gender-related information broadly across early-to-middle layers.  Because CoT templates repeat the condition at multiple token positions, we emphasize matched first-condition-token comparisons rather than claiming the circuit dissolved.

% \subsection{Comparison between simple \& CoT}
% At the MLP site, the first condition-token layer-18 effect under CoT remains comparable to simple prompting, while later condition mentions are much weaker. At the residual site, gender information remains broadly recoverable under both regimes, with a collapse around layer 22. We therefore reject a dissolution interpretation: CoT changes where the MLP effect is strongest within the trace, but does not eliminate early condition-token layer-18 effects or the broader residual encoding. This is further supported by the SAE results in Section~\ref{sec:sae}.
...
In addition, we hoped to glean specific epidemiological and behavioral concepts the model entangles with gender bias, allowing for future improvements in causal interventions. We acknowledge that our results might not be generalizable across all models.
...
To extract the final set of 138 bias-relevant latents, we employed a fixed, multi-stage filtering pipeline to ensure systematic selection. We began with an initial candidate pool of 1,200 latents (the top 100 features across 12 layers, derived from the linear probe rankings on the clinical notes). First, we applied a strict numerical threshold, isolating the top 20 latents per layer based on their absolute probe coefficient. This reduced the pool to 240 highly predictive candidates. These 240 latents then underwent a structured manual semantic review to filter out spurious features, yielding the final dictionary of 138 latents.

%this sentence replaced with the blue text above
% After a manual pass, we extracted 138 gender bias latents distributed across the available layers.

We developed a token-level zero ablation experiment: for each prompt $\times$ condition trace, we ablate individual latents at specific token positions and measure the resulting change in gender logit difference. All experiments use five independent runs with different seeds. Bootstrap confidence intervals summarize uncertainty (see Appendix~\ref{app:sae_notes} for interpretation notes).
...
{Figure~\ref{fig:simple_CI} (Appendix~\ref{app:sae_controls}) shows the top ten latents under simple prompts. To ensure statistical reliability, we apply a minimum sample size filter ($n \ge 25$), excluding low-confidence outliers such as Layer 18 feature 4600 (which showed an inflated mean of 3.47 from only 10 intervention rows). Among the robust findings, Layer 3 feature 84633 again appears prominently (mean 0.65; $n=700$ rows), supporting its influence across various prompting variations. However, the remainder of the top-ten list is distinct from CoT's top-ten list. Layer 7 feature 29873 (mean 0.66) and additional layer-3 latents (F90484, F108381) rank highly under simple prompts but do not appear in the CoT top-ten.}

%old and should replace with above
...
{Control comparisons for simple prompts show clear separation from baselines (see Appendix~\ref{app:sae_controls}, Figure~\ref{fig:simple_control}). Real ablations (mean 0.159) exceeded semantic controls (mean 0.085; $\Delta = 0.074$, $p < 10^{-40}$) and random magnitude-matched controls (mean 0.088; $\Delta = 0.071$, $p < 10^{-40}$). While these $p$-values are orders of magnitude smaller than those observed for CoT ($p \sim 0.0005$), we note that this disparity is heavily driven by sample size (1,557 rows per run for simple prompts versus significantly fewer for CoT). Consequently, rather than indicating that latents are inherently more specific under simple prompts, the extreme significance bounds primarily reflect the much larger evaluation set.}

%old paragraph should replace with above
...
\section{Limitations}
CoT MLP patching uses fixed prompt templates with a forced \texttt{Gender:} scoring suffix rather than model-generated reasoning traces; residual-stream CoT summaries (Appendix~\ref{app:residual}) span longer token sequences where single-position interventions can be harder to interpret. The rewrite score additionally saturates near its ceiling in low-baseline-probability regimes, as with rheumatoid arthritis under Qwen; we report both metrics to mitigate this, but neither alone is sufficient. Under CoT we emphasize first- vs last-condition-token rewrite scores; comparisons that ignore token position or confuse macro- vs per-condition means can misstate whether the early layer-18 effect persists. CoT MLP and residual patching, SAE ablation, and behavioral baselines use Qwen only; OLMo contributes simple-prompt MLP replication only. CoT residual summaries are read from heatmaps rather than aggregated tables. Finally, latent semantic labels are based on manual qualitative inspection and should be interpreted as hypothesis-generating rather than definitive feature semantics.

% \section{Discussion}
% Our results across activation patching, scaled vignette generation, and SAE ablation converge on a consistent answer: CoT does not eliminate gender bias internally, but it does change how that bias is organised and accessible within the model. Under simple prompting, activation patching localises a clear gender signal to MLP layer 18 in both Qwen and OLMo. Under CoT on Qwen, first-condition-token layer-18 MLP effects remain comparable to simple prompting, while later condition mentions are weaker. Residual-stream patching likewise shows that gender-related information remains recoverable across early-to-middle layers. The SAE results provide independent evidence: despite CoT's quantitative suppression of bias-relevant latent activations, they share the same early layer 3 feature that persists across both prompting strategies, and unlike single-position MLP patching in multi-mention CoT templates, SAE ablations do not depend on choosing one condition-token site. Together these findings have three practical implications. First, prompt engineering alone is insufficient as a debiasing strategy for clinical LLMs. Second, evaluation of clinical LLM bias must go beyond behavioral audits, since the RA case illustrates how a condition can appear unresponsive on one metric while retaining a clear gender signal on another. Third, future debiasing work should target the representation level directly, rather than attempting to filter bias through prompting strategies applied at inference time.
% \section{Conclusion}
% Under simple prompts, activation patching localizes gender bias to MLP layer 18 in Qwen and OLMo. Under CoT on Qwen, first-condition-token layer-18 effects remain comparable to simple prompting, while later condition mentions are weaker, and the distributed residual representation remains intact. Ablation of SAE latents demonstrated that both Simple and CoT prompting rely on the same core early-layer latent feature. This suggests that while CoT may alter the visible reasoning trace to appear neutral and medically sound, the model continues to rely on the biased representations. We argue that chain of thought alone is insufficient for deploying LLMs in high-stakes clinical environments, and future work must focus on disentangling demographic information from harmful stereotypes at the representation level.

\section{Discussion and Conclusion}
Our activation-patching, vignette-generation, and SAE-ablation results show that CoT does not eliminate the tested internal gender-bias signal. Instead, it redistributes how that signal is expressed. Under simple prompting, gender information localizes sharply to MLP layer 18 in Qwen and OLMo. Under CoT, this layer-18 effect remains strong at the first condition mention but weakens later, while gender-associated residual-stream representations persist across early and middle layers. SAE ablations further identify the same persistent early-layer feature, L3:F84633, under both prompting strategies. CoT can therefore alter the visible reasoning trace without removing the representation associated with the tested bias.

Three implications follow. First, prompt engineering alone is not sufficient evidence of clinical debiasing, and a neutral-looking reasoning trace should not be treated as proof that demographic dependence has been removed. Second, behavioral evaluation alone can provide false assurance: rheumatoid arthritis shows limited behavioral change while retaining clear internal gender signals. Finally, mechanistic evaluation should complement behavioral audits when assessing mitigation claims in high-stakes clinical settings. This result is diagnostic rather than a validated debiasing intervention, but it motivates representation-level mitigation research before such systems are relied on in consequential clinical workflows.
...
% ========== APPENDIX-REPORTING-SCOPE-START ==========
% Relocated here from the body (old \subsection{Reporting scope}, 2.4) to keep
% the main text under the 9-page limit. Expanded slightly into two \paragraph
% blocks; no numbers changed. Referenced from the results sections via
% \ref{app:reporting}.
\section{Reporting scope and aggregation conventions}
\label{app:reporting}

Rewrite scores in this paper are aggregated at two different levels that are
easy to conflate, so we state the conventions explicitly here.

\paragraph{Macro-means vs.\ per-condition means.} Unless stated otherwise,
headline rewrite-score means are \textbf{macro-averages} over all
prompt$\times$condition cells: 155 for simple prompting (5 conditions $\times$
31 templates) and 100 for CoT on Qwen (5 $\times$ 20). Per-condition bars in
figures are instead means over prompts \emph{within} a single condition. The
two can differ substantially. Asthma averages ${\sim}$0.83 under simple
prompting and ${\sim}$0.94 at the first CoT condition token, while the
corresponding pooled macro-means are only ${\sim}$0.40 and ${\sim}$0.44,
because rheumatoid arthritis and sarcoidosis contribute near-zero scores to the
pool. Per-condition values should therefore never be compared directly against
macro-means.

\paragraph{Token-position aggregation.} Headline layer-18 statistics are
aggregated at the condition-token site: the last condition subtoken for simple
prompts, and the first and last condition mentions reported separately for CoT,
whose templates name the condition at several positions. Full
layer$\times$token sweeps span all token positions. This distinction matters
for the CoT comparison in particular: averaging over all token positions
dilutes the first-mention effect toward ${\sim}$0.01 and can falsely suggest
that the circuit has been removed, which is why we report first- and
last-mention scores separately rather than a single pooled number.
% ========== APPENDIX-REPORTING-SCOPE-END ==========
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{fig11_residual_plateau_qwen (1).png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{fig9_cot_residual.png}
    \caption{CoT residual-stream rewrite scores across conditions (Prompt A, var1).}
    \label{fig:cotresid}
\end{figure}


\section{Top layers for simple prompting}
\label{app:toplayers}

\begin{figure}[h]
    \centering
    \includegraphics[width=.7\linewidth]{fig3_top_layers.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{fig5_mlp_heatmaps.png}
...
\begin{figure}[h]
    ...
    \caption{Mean norm effect scores of top SAE latents for simple prompts, shown with 95\% bootstrap confidence intervals. A minimum sample size filter of $n \ge 25$ was applied to ensure statistical reliability and exclude low-confidence outliers.}
    \label{fig:simple_CI}
\end{figure}
...
\section{SAE Interpretation Notes}
\label{app:sae_notes}
When evaluating the SAE ablation results (Section~\ref{sec:sae}), heatmaps visualize aggregated norm effect values, defined as the shift in the male-female logit difference following intervention, influenced by both effect size and hit density. Token labels (e.g., \texttt{<blank>}, \texttt{Format}) indicate localization sources rather than definitive semantic conclusions; for example, \texttt{<blank>} typically reflects whitespace, while instruction tokens are prompt-specific manifestations. Consequently, we interpret heatmaps and their scores as contextual localization evidence rather than absolute rankings. Finally, the latent interpretations provided rely on manual, subjective qualitative reading.

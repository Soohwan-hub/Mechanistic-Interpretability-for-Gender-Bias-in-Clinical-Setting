...
SCORE_MATRIX_KEYS = (
    "rewrite_scores",
    "logprob_scores",
    "logprob_delta_scores",
    "male_female_logit_delta_scores",
)
...
def run_patch_sweep(
    ...
    Uses MLP down_proj output for patching. Exposes:
      ...
      - male_female_logit_delta_scores =
          (logit_patched(Male) - logit_patched(Female))
          - (logit_unpatched(Male) - logit_unpatched(Female))
    """
    ...
    for start in range(0, n_l, step):
        ...
        if (need_rewrite and corrupted_prob_val is None) or (
            ...
            need_mf_logit_delta and corrupted_mf_logit_diff_val is None
        ):
            with torch.no_grad():
                with llm.generate(max_new_tokens=1) as tracer:
                    with tracer.invoke(corrupted_prompt):
                        ...
                        if need_mf_logit_delta:
                            corrupted_mf_logit_diff_proxy = (
                                logits[0, -1, male_token_id] - logits[0, -1, female_token_id]
                            ).save()
            if need_rewrite:
            ...
            if need_mf_logit_delta:
                corrupted_mf_logit_diff_val = float(
                    _resolve(corrupted_mf_logit_diff_proxy).cpu().float().item()
                )
        ...
        corrupted_mf_logit_diff = (
            corrupted_mf_logit_diff_val
            if corrupted_mf_logit_diff_val is not None
            else 0.0
        )
        ...
        for layer_idx in layer_indices:
            for token_idx in range(token_count):
                with torch.no_grad():
                    with llm.generate(max_new_tokens=1) as tracer:
                        with tracer.invoke(corrupted_prompt):
                            ...
                            if need_mf_logit_delta:
                                patched_mf_logit_diff = (
                                    patched_logits[0, -1, male_token_id]
                                    - patched_logits[0, -1, female_token_id]
                                )
                                mf_logit_delta = patched_mf_logit_diff - corrupted_mf_logit_diff
                                mf_logit_delta_proxy = mf_logit_delta.save()
                if need_rewrite:
                ...
                if need_mf_logit_delta:
                    mf_logit_delta_list.append(float(_resolve(mf_logit_delta_proxy).cpu().float().item()))
    ...
    result: Dict[str, Any] = {
        ...
        "corrupted_male_female_logit_diff": corrupted_mf_logit_diff_val
        if corrupted_mf_logit_diff_val is not None
        else 0.0,
    }
    if need_rewrite:
    ...
    if need_mf_logit_delta:
        result["male_female_logit_delta_scores"] = np.array(mf_logit_delta_list, dtype=float).reshape(n_l, token_count)
    return result
...
def plot_heatmap(
    ...
    if mode == "single":
        fig = px.imshow(
            ...
            labels={"x": "Token", "y": "Layer", "color": color_label},
            x=x_labels,
            y=layer_labels,
            title=title,
        )
    ...
    n_layers = score_matrix.shape[0]
    split_idx = max(1, n_layers // 2)
    chunks = [
        (score_matrix[:split_idx], layer_labels[:split_idx], "_part1"),
        (score_matrix[split_idx:], layer_labels[split_idx:], "_part2"),
    ]
    ...
    if overview_bin_size > 1:
        n_bins = (score_matrix.shape[1] + overview_bin_size - 1) // overview_bin_size
        binned = np.zeros((score_matrix.shape[0], n_bins), dtype=float)
        bin_labels: List[str] = []
        for i in range(n_bins):
            s = i * overview_bin_size
            e = min((i + 1) * overview_bin_size, score_matrix.shape[1])
            binned[:, i] = np.nanmean(score_matrix[:, s:e], axis=1)
        ...
        fig = px.imshow(
            ...
            color_continuous_midpoint=0.0,
            color_continuous_scale="RdBu",
            labels={"x": f"Token bins ({overview_bin_size})", "y": "Layer", "color": color_label},
            x=bin_labels,
            y=layer_labels,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Comma-separated score matrix keys to compute/use. Values: rewrite_scores,logprob_scores,logprob_delta_scores,male_female_logit_delta_scores,all",
    )
    p.add_argument(
        "--model-id",
        type=int,
...
def run_patching(args: argparse.Namespace, run_dir: Path) -> None:
    ...
    for cohort, prompt_id in work_list:
        ...
        try:
            ...
            male_id = int(_validated_gender_token_ids(llm, "Male")[-1].item())
            female_id = int(_validated_gender_token_ids(llm, "Female")[-1].item())
            ...
            sweep = run_patch_sweep(
                ...
                male_token_id=male_id,
                female_token_id=female_id,
            ...
            metadata = {
                ...
                "corrupted_male_female_logit_diff": sweep.get(
                    "corrupted_male_female_logit_diff", 0.0
                ),
                "num_layers": num_layers,
                "num_tokens": n_t,
            ...
            if args.save_heatmaps:
                ...
                for score_key in selected_score_keys:
                    ...
                    try:
                        plot_heatmap(
                            sweep[score_key], token_labels, layer_labels,
                            f"{cohort} prompt{prompt_id} {score_key}",
                            str(plot_dir / f"{cohort}_prompt{prompt_id}_{score_key}"),
                            args.plot_format,
                            mode=args.heatmap_mode,
                            token_window=args.heatmap_token_window,
                            overview_bin_size=args.heatmap_overview_bin_size,
                            color_label=score_key,
                        )
                    except Exception as e:
                        print(f"Plot warning (heatmap) {key} {score_key}: {e}", file=sys.stderr, flush=True)
...
def rebuild_plots_only(args: argparse.Namespace, run_dir: Path) -> None:
    ...
    for p in sorted(artifacts_dir.glob("*.pkl")):
        ...
        if args.save_heatmaps:
            ...
            plot_dir.mkdir(parents=True, exist_ok=True)
            for score_key, matrix in score_matrices.items():
                base_heatmap = plot_dir / (name + f"_{score_key}" + ext)
                ...
                expected_first_tile = plot_dir / (name + f"_{score_key}" + f"_part1_tok0000-{first_window_end:04d}" + ext)
                heatmap_overview = plot_dir / (name + f"_{score_key}" + f"_overview_bin{args.heatmap_overview_bin_size}" + ext)
                ...
                if needs_plot:
                    plot_heatmap(
                        ...
                        layer_labels,
                        f"{name} {score_key}",
                        str(plot_dir / (name + f"_{score_key}")),
                        args.plot_format,
                        mode=args.heatmap_mode,
                        token_window=args.heatmap_token_window,
                        overview_bin_size=args.heatmap_overview_bin_size,
